...
class TestCommand(unittest.TestCase):
    ...
    def test_create_object(self):
        with patch('builtins.input', side_effect=["create MyClass name=\"My_little_house\" age=30", "quit"]):
            self.console.cmdloop()
            # Replace "MyClass" and "30" with the actual class name and value you expect
            self.assertTrue("MyClass" in self.console.do_create_output)
            self.assertTrue("30" in self.console.do_create_output)
            self.assertIn("Created MyClass object with parameters:", self.console.do_create_output)

    def test_missing_class_name(self):
        with patch('builtins.input', side_effect=["create", "quit"]):
            self.console.cmdloop()
            self.assertIn("** class name missing **", self.console.do_create_output)

    def test_invalid_class_name(self):
        with patch('builtins.input', side_effect=["create InvalidClass name=\"My_little_house\"", "quit"]):
            self.console.cmdloop()
            self.assertIn("** class doesn't exist **", self.console.do_create_output)

    def test_invalid_parameter_syntax(self):
        with patch('builtins.input', side_effect=["create MyClass name=My_little_house", "quit"]):
            self.console.cmdloop()
            self.assertIn("Skipping invalid parameter:", self.console.do_create_output)
